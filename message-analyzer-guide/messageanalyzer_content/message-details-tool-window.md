...
## Using the Details Tool Window Features  
 ...
 You can also filter any column of data in the **Details** window to isolate specific values by applying a **Column Filter**, similar to the way you do this in the **Analysis Grid** viewer, as described in [Filtering Column Data](../messageanalyzer_content/filtering-column-data.md). To apply a **Column Filter**, you must click the **Show Column Filter Row** icon just below the **Details** window toolbar and then enter text in one of the columns to filter the data. Thereafter, only the fields that contain the text value that you specified in a particular column will display data. However, in some cases, a search result value for a field will display only if the tree containing the field is open. The column filtering feature provides a convenient way to group data based on field names, values, types, and so on.  
...
-   **Track ‘\<entityName>’** — enables you to track the value of a particular field or property in the **Details** window, depending on which of the following **Details** toolbar buttons is selected. Note that you can hover over the **Details** toolbar buttons with your mouse to display a tooltip that identifies them:  
    ...
    > [!TIP]
    >  If you have the [Selection Tool Window](../messageanalyzer_content/selection-tool-window.md) set for tracking messages, you can conveniently backtrack to previous message selections to view tracked field and property values.  
...
## Changing Analysis Perspectives with Details Operations  
 As described earlier, when a particular viewer is in focus, you can right-click any **Details** window field or property and select an appropriate context menu command (see [Using the Details Tool Window Features](../messageanalyzer_content/message-details-tool-window.md#BKMK_UsingDetailsTracking)), to change your analysis perspective. You can also use corresponding toolbar buttons to achieve the same result in the following scenarios:  
